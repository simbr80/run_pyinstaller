...
def create_exe():
    ...
    if dist_path.exists():
        delete = input("Izbrisi obstojeco exe distribuicjo (d/n) ?")
        if delete == "d":
            ...
            commands = [f'{working_file_to_dir}\\env\\Scripts\\activate.bat',
                        f'{working_file_to_dir}\\env\\Scripts\\pyinstaller.exe --onefile {str(new_file_path)}',
                        f'{working_file_to_dir}\\env\\Scripts\\deactivate.bat'
                        ]
